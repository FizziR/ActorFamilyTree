...
object Main{
  ...
  val graph: Graph[ClosedShape.type, NotUsed] = GraphDSL.create(){ implicit builder:
    ...
    val splitStringsInTuples: FlowShape[String, (String, String, String)] = builder.add(Flow[String].map(i => {
      i.split("\\%+") match {
        case Array(timestamp: String, author: String, message: String) => (timestamp, author, message)
        case _ => ("", "", "")
      }
    }))
    val output: SinkShape[(String, String, String)] = builder.add(Sink.foreach[(String, String, String)](println))
